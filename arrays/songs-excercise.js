...
// if(song.songID === 10){        // the (find) replaces all of this code
//     console.log(song);
//     return
//}
...
// let songtitle = songs.map((songs) => songs.title  );
...
// let oliviaSongs = songs.filter((song) => song.artist === "Olivia Rodrigo");
//     console.log(oliviaSongs);
...
// let songsByArtist = songs.map((song) => song.title);

// console.log(songs.map((song) => `${song.title} by ${song.artist}`));
...
// let songYear = songs.filter(song => song.year === 2021);
// console.log(songYear);

// 10. **Find the first song of a specific genre**:
//     - Use the `find` method to find the first song in the
